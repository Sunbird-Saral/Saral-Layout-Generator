...
const FormBuilder = ({
  ...
  const [boxes, setBoxes] = useState([
    { key: Date.now(), x: 45, y: 45, width: 40, height: 30, textsize: 20 },
  ]);
      ...
      <div id="print-this" className="area">
        {boxes.map((box) => (
          ...
          >
            ...
            <button
              onClick={() => addbox(box, "right")}
              className="add-button right"
            >
              +
            </button>
            <button
              onClick={() => addbox(box, "bottom")}
        ...
        <div className="tool-area">
          ...
          <DownloadPDF
            boxes={boxes}
            blackdots={blackdots}
            setBlackdots={setBlackdots}
            handleDesignComplete={handleExportComplete}
            orientationOfForm={selectedOption}
          />
          <div>
            <button
              className={
                isDesignComplete ? "download-button" : "button-disabled"
              }
        ...
        <BlackDotComponent
          blackdots={blackdots}
          updateblackdots={updateSharedState}
        />
      </div>
    </div>
  );
